...
def test_end_to_end():  # создаем данные нового пользователя в формате, который задан на сайте
    new_user = {
        'username': 'my user',
...
# Создаем нового пользователя
    ...
    assert response.status_code == 200  # проверяем код успешного выполнения post
...
# Проверим, что создали
    ...
    assert response.status_code == 200  # проверяем код успешного выполнения get
    # print(response.json())  # test_api_08.py::test_end_to_end {'username': 'my user'}
    # print('get==>', response.headers)

# Проведем авторизацию нового пользователя
    auth_data = {  # создаем словарь с данными нового пользователя, чтобы пройти авторизацию
        'email': new_user['email'],  # берем именно тот email, который сгенерировали при создании new_user
        'password': '12345'
    }

    response = requests.post(f'{url}user/login', data=auth_data)  # запрос на авторизацию  endpoint is login
    assert response.status_code == HTTP_code, 'wrong status code'  # если авторизация успешна, д.б. статус 200
    # print(response.headers)  # появились куки и токен: 'Set-Cookie', 'x-csrf-token'

# куки и токен понадобятся для put и delete, поэтому сохраним их с помощью их ключей 'x-csrf-token' и 'Set-Cookie':

    token = response.headers['x-csrf-token']  # токен (знак, жетон) — это электронный ключ для доступа
    auth_sid = response.cookies['auth_sid']  # фрагмент данных для идентификации и хранения данных о пользователе
    # print(token, auth_sid)

    new_user_update = {
        'username': 'my user updated'  # создаем словарь с обновленными данными
    }
...
# Проверим, что данные пользователя изменились:
    response = requests.get(f'{url}user/{user_id}')
    assert response.status_code == HTTP_code

    response_data = response.json()
    assert 'my user updated' in response_data.values()
    # print('response_data==>', response_data)  # response_data==> {'username': 'my user updated'}


# Удалим данного пользователя:
    response = requests.delete(f'{url}user/{user_id}',
                               headers={'x-csrf-token': token},
                               cookies={'auth_sid': auth_sid})
    # print(response.headers)  # токена и куки уже нет

# Для проверки попытаемся опять получить информацию о нашем пользователе:
    response = requests.get(f'{url}user/{user_id}')
